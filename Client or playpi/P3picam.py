...
# It takes 2 pictures with some delay between them,
# then it compares them pixel by pixel, and depending on 
# the chosen sensitvity it detects the change in these pixels 
# and take the decision that there is motion or not
...
forceCapture = True    # whether to force an image to be captured every forceCaptureTime seconds (values True or False)
forceCaptureTime = 60 * 60    # Once an hour
filepath = "/home/pi/picam"    # location of folder to save photos
filenamePrefix = "capture"    # string that prefixes the file name for easier identification of files.
diskSpaceToReserve = 40 * 1024 * 1024    # Delete oldest images to avoid filling disk. How much byte to keep free on disk (40mb here).
...
# Fn that returns wheter there is motion or not
def motion(): 
    # Get first image
    ...
    while (True):
        ...
        # Here we compare the 2 pictures pixel by pixel
        for z in range(0, testAreaCount): # = xrange(0,1) with default-values = z will only have the value of 0 = only one scan-area = whole picture
            for x in range(testBorders[z][0][0]-1, testBorders[z][0][1]): # = xrange(0,100) with default-values
                for y in range(testBorders[z][1][0]-1, testBorders[z][1][1]):   # = xrange(0,75) with default-values; testBorders are NOT zero-based, buffer1[x,y] are zero-based (0,0 is top left of image, testWidth-1,testHeight-1 is botton right)
                    ...
                    if (changedPixels > sensitivity): # ************ the decision for motion ********** 
